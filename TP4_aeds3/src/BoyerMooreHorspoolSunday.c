...
int** cria_tabela_ (char *padrao, int tamanho) {
	...
	for (i = 0; i < tamanho; i++) { // percorrer o padrao e verificar se existe alguma letra repetida
		if (is_in_array_(aux, padrao[i]) == -1) {
	...
	for (i = 0; i < tamanho; i++) {
		tabela[is_in_array_(aux, padrao[i])][1] = (tamanho - 1) + 1 - i;
	}
	free(aux);
	return tabela;
}
...
void bmhs (char *texto, char *padrao) {
	...
	int **tabela = cria_tabela_(padrao, tamanho_padrao);
	/*
	printf("%s\n", padrao);
	for (i = 0; i < qnt_letras_tab; i++) {
	...
	for (i = tamanho_padrao - 1; i < tamanho_texto - 1;) {
		...
		for (j = tamanho_padrao - 1; j >= 0 && trigger == 0; j--) {
			if (padrao[j] != texto[i + (j - (tamanho_padrao - 1))]) {
				i += deslocamento_(tabela, texto[i + 1], tamanho_padrao);
				trigger = 1;
			}
			else if (j == 0) {
